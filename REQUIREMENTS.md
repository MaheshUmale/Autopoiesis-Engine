...
To achieve seamless, lock-in-free integration across tools like **VS Code, Claude Desktop, Cursor, Kilocode, and OpenCode**, the framework uses two open industry standards as its integration interface: **Model Context Protocol (MCP)** and **OpenTelemetry (OTEL) / CLI Hooks**.

You **do not** run the framework inside a single platform’s proprietary ecosystem, nor do you write custom extension code for every IDE. Instead, the framework runs as an **independent local background daemon/service** on your system. IDEs and coding agents simply connect to it via universal interfaces.

---

### How the Framework Integrates Across Environments

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                          IDE / AGENT FRONTENDS                              │
│   [ VS Code / Cursor ]   [ Claude Desktop ]   [ Kilocode / OpenCode ]   [ CLI ] │
└───────────────────────────┬───────────────────┬─────────────────────────────┘
                            │                   │
                     (MCP via Stdio/HTTP)   (CLI / Telemetry)
                            │                   │
                            ▼                   ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                     AUTOPOIESIS ENGINE DAEMON (Background)              │
│  • MCP Server Router            • Temporal Workflow Harness                 │
│  • 3-Tier Skill Registry         • Sandboxed Self-Healing Engine             │
└─────────────────────────────────────────────────────────────────────────────┘

```

#### 1. Integration via Model Context Protocol (MCP)

MCP is an open standard supported across major AI coding tools (Claude Desktop, Cursor, VS Code, Kilocode, OpenCode, Continue.dev).

* **How it works:** The `Autopoiesis Engine` runs a lightweight local MCP server (`autopoiesis-mcp`).
* **Tool Discovery:** When your IDE connects to the engine via MCP, the engine automatically exposes all active micro-skills and composite templates as tools.
* **Execution:** When Claude or Cursor decides to execute a tool (e.g., `fetch_upstox_data`), it sends an MCP tool call request to the local daemon, which handles execution, sandboxing, and self-healing asynchronously.

#### 2. Switching IDEs/Models Without Lock-In

Because all tool definitions, memory, history, and DAG templates live inside the **local Autopoiesis engine** (and not inside VS Code extensions or Anthropic's cloud):

* **Zero Data Loss:** If you move from Claude Desktop to Cursor or Kilocode, you bring your `.autopoiesis/` folder and local engine with you.
* **Identical Capability Set:** Your new agent picks up the exact same 3-Tier Skill Registry, cached DAG templates, and active namespaces immediately upon starting the MCP server.

---

### One-Command Cross-Platform Installation Spec

To ensure easy installation across Windows (PowerShell), macOS, and Linux without native dependencies:

#### 1. Distribution via `uv` / `pipx`

The entire engine is packaged as a standalone Python CLI tool using `uv` or `pipx`.

```bash
# Single command installation
pipx install autopoiesis-engine

# Or via uv (recommended for speed)
uv tool install autopoiesis-engine

```

#### 2. Auto-Configuration CLI (`autopoiesis init`)

Running `autopoiesis init` inside your project directory sets up the environment automatically:

1. Detects installed IDEs (VS Code, Cursor, Claude Desktop).
2. Generates/updates the respective MCP configuration files automatically:
* **Claude Desktop:** `~/Library/Application Support/Claude/claude_desktop_config.json`
* **Cursor:** `.cursor/mcp.json`
* **VS Code / Continue:** `.vscode/mcp.json`


3. Initializes local SQLite metadata storage, Qdrant vector index, and the `.autopoiesis/` local state workspace.

---

### Universal MCP Configuration Schema (`mcp.json`)

This single standard configuration snippet works across **VS Code, Cursor, Kilocode, OpenCode, and Claude Desktop**:
...
```json
{
  "mcpServers": {
    "autopoiesis-engine": {
      "command": "autopoiesis",
      "args": ["serve", "--project-config", "./project.yaml"],
      "env": {
...
Add this section to the master **Autopoietic Agent Framework (AAF)** specification:

```markdown
## 12. Cross-Environment Integration & Client Architecture

### A. Non-Proprietary Integration Architecture
1. The framework MUST operate as an autonomous, OS-level daemon (`autopoiesis serve`).
2. Tools and dynamic capabilities MUST be exposed exclusively using the Model Context Protocol (MCP) over stdio or local SSE (Server-Sent Events) HTTP transport.
3. No environment-specific or proprietary extension APIs (e.g., VS Code Extension API) shall be required for core functionality.

### B. Auto-Installer & System Configurator (`autopoiesis init`)
The CLI module `autopoiesis.cli` must implement auto-discovery for client configurations:
- Detect operating system (`sys.platform`).
- Locate config paths for installed clients (VS Code, Cursor, Claude Desktop, Kilocode).
- Inject the standard `autopoiesis-engine` MCP entrypoint into client JSON configuration files without overwriting existing client settings.
...
**Target Audience:** Autonomous AI Coding Agents (Cursor, Claude Code, Windsurf, Devika)

**Protocol Standards:** Model Context Protocol (MCP), Temporal.io Distributed Harness, OpenTelemetry (OTEL)

**System Objective:** Build a self-evolving, multi-tenant agent execution framework that autonomously writes, tests, parameterizes, caches, self-heals, and orchestrates micro-skills into composite workflow DAGs using a biological paradigm (**DNA $\rightarrow$ RNA $\rightarrow$ Protein Synthesis**).
...
* **DNA Primitives (Micro-Skills):** Atomic, single-function Python modules with strict Pydantic inputs/outputs. Fully decoupled from runtime variables (parameterized).
* **Proteins (Composite DAGs):** Workflows built dynamically by assembling micro-skills into Directed Acyclic Graphs.
* **Autopoiesis (Self-Healing & Synthesis):** The framework anticipates missing tools via look-ahead parsing, generates the missing logic, tests it in an isolated sandbox, repairs runtime errors via telemetry loops, and saves successful graphs as parameterized templates.
...
```
[ Incoming Project Intent (project.yaml / prompt) ]
                           │
                           ▼
   [ 1. Look-Ahead Spec Parser ] ──(Extracts Scope & Execution Pipeline)
                           │
...
The framework prevents semantic bloat and context-window exhaustion by segmenting tools into a strict three-tier hierarchy indexed by SQLite (relational) and Qdrant (vector) databases.
...
```json
{
  ...
  "inputs": {
    "type": "object",
    "properties": {
      "instrument_key": { "type": "string", "example": "NSE_EQ|INE002A01018" },
      "interval": { "type": "string", "enum": ["1minute", "5minute", "30minute", "day"] },
      "from_date": { "type": "string", "format": "date" },
      "to_date": { "type": "string", "format": "date" }
    },
    "required": ["instrument_key", "interval", "from_date", "to_date"]
  },
  "outputs": {
    "type": "object",
    "properties": {
...
The system resolves required context before execution starts to filter vector retrieval.
...
required_pipeline_intent: |
  Fetch 1-minute historical candles from Upstox for last 30 days,
  calculate 20-period Volume SMA, and store result into PostgreSQL table 'options_volume'.

```

### Logic Sequence

1. Parse `required_pipeline_intent` to extract semantic execution nodes.
2. Search Qdrant vector database using payload filter: `namespace IN active_namespaces`.
3. If vector similarity for a step is below `0.85`, dispatch an asynchronous Temporal activity to the **Skill Writer** to generate the tool *before* the Orchestrator reaches that execution step.
    ...
    "nodes": [
      {
        ...
        "skill_id": "trading.upstox.fetch_historical",
        "args": {
          "instrument_key": "{{ parameters.instrument }}",
          "interval": "{{ parameters.timeframe }}"
        }
      },
      {
        "id": "step_2",
        "skill_id": "global.parsers.json_to_df",
        "args": { "raw_data": "{{ step_1.output }}" }
      },
      {
        "id": "step_3",
        "skill_id": "global.db.postgres_upsert",
...
Timeouts scale dynamically based on input payload size to prevent false positives on large data parsing:

$$\text{Timeout}_{\text{total}} = 5.0 + \left( \frac{\text{Payload}_{\text{bytes}}}{1,048,576} \times 2.0 \right)$$


*(Base 5 seconds, plus 2 seconds per MB of input data).*

### B. Diagnostic Decision Tree & Healing Matrix

When an execution fails, evaluate the trace using this sequence:

```
                        ┌───────────────────────────────┐
                        │      Skill Failure Caught     │
                        └───────────────┬───────────────┘
                                        │
                ...
                ┌───────────────────────┴───────────────────────┐
                ▼                                               ▼
     (Invalid Input Payload)                         (Payload Valid)
                │                                               │
     Blame Upstream Node                             [ Step 2: Sandbox Isolation ]
     (Do NOT alter Skill)                                       │
                                                        Execute with Mock Inputs
                                                                │
                                                ┌───────────────┴───────────────┐
                                                ▼                               ▼
                                       (Execution Timeout)              (Code Error/Exception)
                                                │                               │
                                       Calculate Payload Size            Pass Stack Trace to Writer
                                                │                               │
                                       Scale Timeout Formula             Generate Code Patch & Re-Test

```

1. **Schema Validation Fails on Input:** Issue lies with Upstream Node. Do not mutate current skill code; trigger upstream pipeline repair.
2. **Sandbox Pass $\rightarrow$ Live Fail (Timeout/OOM):** Environmental Resource failure. Auto-patch Python code to use streaming/chunking generators instead of holding objects in memory.
3. **Sandbox Pass $\rightarrow$ Live Fail (HTTP 429/500):** Network error. Auto-patch skill with exponential backoff decorator.
4. **Sandbox Fail (TypeError / SyntaxError):** Logic error. Route OTEL trace, `stderr`, and source code back to Skill Writer for a hotfix (Maximum 3 retries). Tool execution errors must conform to MCP standard by returning `isError: true`.

---

## 7. Feature 4: Temporal.io Distributed Harness Integration

To manage multi-worker concurrency, timeouts, and state tracking, the engine uses **Temporal.io**.
...
* **Constraint:** Workflows must be strictly deterministic (no random numbers, system clock calls, or un-mocked I/O inside `@workflow.run`).

---

## 8. Feature 5: Platform-Agnostic Execution Layer (Windows / Linux / macOS)

Agents must NEVER generate OS-specific shell strings (e.g., `bash -c` or `subprocess.run("ls")`). The engine enforces a `PlatformAdapter`.
...
For developers managing background workers programmatically, the system provides a `tmux` interface using `libtmux`.
...
## 10. Feature 7: Observability & Inter-Node State Pipeline

### A. OpenTelemetry & Trace Snapshots

Every step emits OTEL spans and appends state snapshots to `.autopoiesis/traces/{execution_uuid}.json`.

### B. Inter-Node State Thresholding

To prevent exceeding Temporal's 2MB–4MB activity payload limit:

* **Small Payloads (< 100 KB):** Passed inline via Temporal activity return values.
* **Large Payloads ($\ge$ 100 KB):** Serialized to disk/object storage (`.autopoiesis/staging/{execution_id}/{node_id}.parquet`), returning a payload pointer:
`{"_storage_type": "file", "path": "..."}`
Downstream runtimes hydrate this path back into memory automatically.

---

## 11. Feature 8: AST Deduplication & MCP Integration

### A. Normalized AST Deduplication

To strip comments, variable names, and docstrings before checking structural code identity:
...
def get_normalized_ast_hash(python_code: str) -> str:
    """Strips docstrings, comments, and variable identifiers to generate structural AST fingerprint."""
    tree = ast.parse(python_code)
    for node in ast.walk(tree):
        if isinstance(node, (ast.FunctionDef, ast.ClassDef, ast.AsyncFunctionDef)):
            node.name = "_"
...
### B. Cross-IDE Integration via Local MCP Daemon

The framework runs as an OS-level daemon (`autopoiesis serve`) exposing tools over stdio/SSE using Model Context Protocol (MCP).
...
1. **Initialize Project:** Setup `pyproject.toml` with `temporalio`, `qdrant-client`, `pydantic>=2.0`, `libtmux`, `mcp`, and `opentelemetry-sdk`.
2. **Database:** Initialize SQLite for relational mappings and Qdrant (local memory mode) for vector similarity.
3. **Execution Layer:** Implement `PlatformAdapter` and `SandboxExecutor` first.
4. **Temporal Workflows:** Map logic in Section 7 exactly. Handle failures via `RetryPolicy`, routing terminal failures to a `SelfHealingWorkflow`.
5. **No Placeholders:** Write complete, production-ready Python code. Implement explicit AST-hash checking for deduplication.
